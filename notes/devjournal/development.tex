...
\begin{definition}[Ray map]
   ...
   \[
        R_{\lambda}(r) = \left\langle R_\lambda^x, R_\lambda^y \right\rangle = \langle \cos r_i, \sin r_i \rangle
   \]
   Where $\lambda$ is the \emph{maximum sensory depth} such that
   \[
        \left|R_{\lambda}(r)\right|^2 = \left(R_\lambda^x\right)^2 + \left(R_\lambda^y\right)^2 = \lambda^2
   \]
\end{definition}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.58\textwidth]{img/mcl_simulation.png}
...
\par A failsafe way to ensure that all entities within the sensory field are recognized is by introducing adjacent chunk loading (henceforth ACL)
